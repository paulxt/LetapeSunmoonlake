...
import GpxDownload from '../../assets/gpx/gpx-104k.gpx';
const RouteGpx = ({language}) => {
    return (
      <section className="route-gpx">
        <h5><span>{language === "Chinese" ? "GPX地圖" : "GPX Map"}</span></h5>
        <Container>
          <Row className='align-items-center'>
            <Col md={3}>
              <h3 className='text-center'><a href={GpxDownload} download={'letape-sunmoonlake-104k-gpx'} >{language === "Chinese" ? "★點我下載GPX檔★" : "★Tap to download GPX★"}</a></h3>
            ...
            <Col md={9} className='gpx-map'>
              <iframe title="104kTheRace"src="https://ridewithgps.com/embeds?type=route&id=47303686&title=104k%20The%20Race&sampleGraph=true&distanceMarkers=true&showPhotos=true"
                ></iframe>
            </Col>
          </Row>
        </Container>
